...
namespace VirtualPrinter.ViewModels
{
	public class StoredImageViewModel : BindableBase
	{
		...
		{
			this.EventAggregator.GetEvent<ViewMetaDataEvent<StoredImageViewModel>>().Publish(new() { Item = this, Action = ViewMetaDataArgs<StoredImageViewModel>.ActionType.Warnings });
			return Task.CompletedTask;
		}

		private Task ViewImageAsync()
		{
			this.EventAggregator.GetEvent<ViewMetaDataEvent<StoredImageViewModel>>().Publish(new() { Item = this, Action = ViewMetaDataArgs<StoredImageViewModel>.ActionType.Image });
			return Task.CompletedTask;
		}
	}
}

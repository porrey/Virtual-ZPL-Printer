...
{
	internal class HsRequestHandler : TemplateRequestHandler
	{
		public HsRequestHandler(ILogger<HsRequestHandler> logger, IEventAggregator eventAggregator, ILabelService labelService, IImageCacheRepository imageCacheRepository)
		...
		protected override Task<bool> OnCanHandleRequestAsync(string requestData)
		{
			...
			if (requestData != null && requestData.ToUpper().StartsWith("~HS"))
			{
		...
		protected override Task<(bool, string)> OnHandleRequest(IPrinterConfiguration printerConfiguration, ILabelConfiguration labelConfiguration, string requestData)
		{
			...
			//int width = (int)(labelConfiguration.Dpmm * new Length(labelConfiguration.LabelWidth, labelConfiguration.Unit).ToUnit(LengthUnit.Millimeter).Value);
